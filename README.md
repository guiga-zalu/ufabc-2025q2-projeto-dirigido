...
**Cada seção deve ter sua justificativa própria.**
...
- Listas não abrangentes. Apenas exemplificadora.
- Descrever a mecânica de inclusão de algoritmos: como pesquisei (que strings de texto usei e onde), etc.

#### Escolha de algoritmos de codificação

- Usei png, qoi e webp sem perdas.
  - Razão:
    - Formatos comuns em computação (arquivamento, web)
    - Implementações acessíveis
    - Exemplificar formatos de codificação sem perdas
- Usei webp e jpeg com perdas, ambos nas qualidades 90%, 80%, 50% e 15%.
  - Razão:
    - Formatos comuns em computação (fotografia, web)
    - Implementações acessíveis
    - Exemplificar diferentes qualidades de imagens
- Usei png após processar a imagem com a ferramenta de quantização `pngquant`, com 128 e 256 cores.
  - Razão:
    - Exemplificar técnicas de codificação e/ou quantização.
...
- Independência quanto ao formato de píxeis da imagem (RGB, YUV, Luma etc.).
- Implementação acessível, que seja fácil de implementar e capaz de ser executada com baixo uso de recursos.
- Resultado em formato de um número escalar, dadas duas imagens a serem comparadas.

Os tipos de algoritmos foram separados em duas categorias:

1. Hashes, que convertem imagens em um valor numérico, que pode ser comparados diretamente, e
...
- aHash[^hackerfactor_kind_of_like_that]
- dHash[^hackerfactor_kind_of_like_that]
- pHash[^hackerfactor_looks_like_it][^library_jimp]
...
No entanto, conforme análise dos dados, mesmo com a desconsideração das distâncias cujo resultado foi zero, a maior média encontrada foi de $2,4\%$ (vezes $64$, é uma vaariação de cerca de $1,5\%$ bits), o que indica que os algoritmos de *hash* escolhidos não são adequados para propriamente representar divergências entre imagens causadas por artefatos de compressões, pois estes algoritmos visam representar similaridades estruturais entre as imagens[^hackerfactor_kind_of_like_that].

##### Métricas

Os algoritmos de *métricas* escolhidos foram os seguintes:
...
- $\mathrm{MAE}$ (*Mean Absolute Error*), e $\mathrm{MSE}$ (*Mean Squared Error*), por sua classicidade literária na área de análise de erros em sinais.
  - $$
    \begin{aligned}
        \mathrm{MAE}(x, y) &= \frac{1}{n}\sum_{i=1}^n\left|x_i - y_i\right| \\
        \mathrm{MSE}(x, y) &= \frac{1}{n}\sum_{i=1}^n(x_i - y_i)^2 \\
    \end{aligned}
    $$
- $\mathrm{SSIM}$ (*Structural Similarity Index Measure*)[^ssim], que analisa a similaridade estrutural entre duas imagens, tendo a definição de similaridade como a composição dos fatores $l$ (luminosidade), $c$ (contraste) e $s$ (estrutura).
  - $$
    \mathrm{SSIM}(x, y)
    = {
...
- GMSM (*Gradient Magnitude Similarity Mean*) e GMSD (*Gradient Magnitude Similarity Deviation*)[^arxiv_gmsd] (definição da implementação conforme artigo).

### ~~Execução sistemática~~ Renomear.

Programa de codificação, decodificação e comparação, e registro de resultados.

- Justificativa técnica: da linguagem.
  - Escolhi Rust, pois:
    - tenho familiaridade;
    - possui módulos já estabelecidos para codificação de imagens nos formatos escolhidos;
    - possui técnicas confiáveis de paralelização, para encurtar o tempo de execução do programa.
- Justificativa técnica: de log em texto.
  - simples de processar;
  - outras soluções, como um banco de dados, necessitariam de uma estrutura pouco maleável, e resiliente para acesso de escrita em paralelo.

Programa de análise de métricas:

- Justificativa técnica: da linguagem.
  - Escolhi Python, pois:
    - tenho familiaridade;
    - é prática para prototipagem e processamento de texto.
...
| WebP ($q = 90\%$) | Sim | $\color{gray} 120\%$ | $63$ ms | $1,66$ | $99,957$ | $99,03$ | $94,6$ | $15,6$ |
...
| JPEG ($q = 90\%$) | Sim | $\color{gray} 168\%$ | $\color{gray} 540$ ms | $0,89$ | $99,98$ | $99,748$ | $98,27$ | $8,8$ |
| JPEG ($q = 80\%$) | Sim | $\color{gray} 124\%$ | $\color{gray} 400$ ms | $1,27$ | $99,963$ | $99,55$ | $97,2$ | $10,8$ |
...
| PNG ($256$ cores) | Sim | $\color{gray} 310\%$ | $\color{gray} 360$ ms | $3,3$ | $99,918$ | $97,5$ | $94,6$ | $16,2$ |
| PNG ($128$ cores) | Sim | $\color{gray} 370\%$ | $350$ ms | $2,5$ | $99,953$ | $98,62$ | $95,7$ | $14,1$ |
...
| WebP ($q = 90\%$) | Sim | $50\%$ | $2,8$ ms | $1,7$ | $24,5$ | $99,954$ | $99,13$ | $90,6$ | $22,5$ |
...
| JPEG ($q = 90\%$) | Sim | $85\%$ | $\color{gray} 40$ ms | $1,77$ | $26,7$ | $99,91$ | $98,9$ | $94,7$ | $17,1$ |
...
| PNG ($256$ cores) | Sim | $\color{gray} 130\%$ | $14$ ms | $1,4$ | $24$ | $99,872$ | $99,66$ | $97,1$ | $11,3$ |
| PNG ($128$ cores) | Sim | $\color{gray} 150\%$ | $16$ ms | $1,1$ | $18$ | $99,984$ | $99,8$ | $98,1$ | $8,8$ |
...
| WebP ($q = 90\%$) | Sim | $86\%$ | $14$ ms | $1,14$ | $$ | $99,952$ | $99,18$ | $99,2$ | $16,3$ |
...
| JPEG ($q = 90\%$) | Sim | $\color{gray} 142\%$ | $\color{gray} 117$ ms | $0,34$ | $$ | $99,9913$ | $99,889$ | $98,79$ | $7,9$ |
...
| PNG ($256$ cores) | Sim | $\color{gray} 440\%$ | $53$ ms | $0,14$ | $$ | $99,9971$ | $99,969$ | $99,8$ | $2,4$ |
| PNG ($128$ cores) | Sim | $\color{gray} 580\%$ | $52$ ms | $0,3$ | $$ | $99,99996$ | $99,998$ | $99,99$ | $8$ |
...
3. Unidade: $\%$. Com exceção da $\mathrm{MSE}$, todas as métricas foram normalizadas corretamente para $\left[0; 1\right]$.
4. Não foi possível encodificar em QOI, devido a algumas imagens serem em escala de cinza, e não em RGB ou RGBA.

- ?
- Definir mecânica de comparação.
...
Até 29/07, selecionar *datasets* de comparação 
